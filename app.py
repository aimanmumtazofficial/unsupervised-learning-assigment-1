...
# The dataset is loaded automatically from a public GitHub mirror
# of the UCI Online Retail dataset so deployed users need no CSV.
DATASET_URL = (
    "https://raw.githubusercontent.com/dsrscientist/dataset1/"
    "master/online_retail.csv"
)

# Fallback: users can still upload their own CSV if auto-load fails
uploaded_file = st.file_uploader(
    "📂  (Optional) Upload your own Online Retail CSV — "
    "or the dataset loads automatically below",
    type=["csv"],
    help="Leave blank to use the built-in dataset loaded automatically from the web."
)

def load_dataset(source):
    """Load and fix column names from a file path, URL, or file-like object."""
...
if st.session_state.df is None:
    if uploaded_file is not None:
        # User provided their own file — use it
        try:
            st.session_state.df = load_dataset(uploaded_file)
            st.success("Dataset loaded from your uploaded file.")
        except Exception as e:
            st.error(f"Could not read uploaded file: {e}")
    else:
        # Auto-download from public URL — no action needed from visitor
        with st.spinner("Loading dataset automatically from the web... please wait."):
            try:
                st.session_state.df = load_dataset(DATASET_URL)
                st.success(
                    "Dataset loaded automatically! "
                    "No file upload required — just scroll down and click **Clean & Aggregate Data**."
                )
            except Exception as e:
                st.error(
